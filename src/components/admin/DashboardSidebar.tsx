...
import { LayoutDashboard, FileText, Users, Mail, Home, ChevronLeft, Sparkles, X } from "lucide-react";
...
const DashboardSidebar = ({
  ...
  return <>
      ...
      {/* Sidebar */}
      <aside className={cn("fixed left-0 top-0 z-50 h-screen bg-dashboard-sidebar border-r border-dashboard-border transition-all duration-300 flex flex-col",
    // Desktop behavior
    ...
    mobileOpen ? "translate-x-0 w-72" : "-translate-x-full w-72")}>
        ...
        <div className="flex items-center justify-between p-5 h-16 border-b border-dashboard-border">
          <div className="flex items-center gap-3">
            ...
            {(!collapsed || mobileOpen) && <span className="font-semibold text-lg tracking-tight">
                PeakFlow
              </span>}
          </div>
          {/* Mobile close button */}
          <button onClick={() => setMobileOpen(false)} className="lg:hidden p-2 rounded-xl hover:bg-dashboard-bg transition-colors">
        ...
        <div className="p-4 border-t border-dashboard-border space-y-1">
          {bottomItems.map(item => <Link key={item.title} to={item.href} onClick={handleNavClick} className="flex items-center gap-3 px-3 py-3 rounded-xl text-muted-foreground hover:bg-dashboard-bg hover:text-foreground transition-all duration-200">
        ...
        <button onClick={() => setCollapsed(!collapsed)} className="hidden lg:flex absolute -right-3 top-20 h-6 w-6 rounded-full bg-dashboard-card border border-dashboard-border items-center justify-center shadow-soft hover:shadow-soft-lg transition-all">
